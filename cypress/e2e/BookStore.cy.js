...
describe('Books Section', () => {
  beforeEach(() => {
    cy.intercept('GET', 'http://localhost:5173/Books', { fixture: 'books.json' }).as('getBooks');
    cy.visit('/books');
    cy.wait('@getBooks');
  });
...
describe('Books Section', () => {
  ...
  it('should display "No books available" message when no books are present', () => {
    cy.intercept('GET', 'http://localhost:5173/Books', { fixture: 'emptyBooks.json' }).as('getEmptyBooks');
    cy.visit('/books');
...
describe('Books CRUD Operations', () => {
  beforeEach(() => {
    ...
    cy.request('POST', 'http://localhost:5173/upload-book', {
      bookname: 'Test Book',
      author: 'John Doe',
  ...
  it('should update an existing book', () => {
    ...
    cy.request('GET', 'http://localhost:5173/Books').then((response) => {
      const firstBookId = response.body.books[0]._id;

      cy.request('PUT', `http://localhost:5173/UpdateBook/${firstBookId}`, {
  ...
  it('should delete an existing book', () => {
    ...
    cy.request('GET', 'http://localhost:5173/Books').then((response) => {
      ...
      cy.request('DELETE', `http://localhost:5173/delete-book/${firstBookId}`);
...
describe('MongoDB Interactions', () => {
  beforeEach(() => {
    ...
    cy.intercept('GET', 'http://localhost:5173/all-books', { fixture: 'books.json' }).as('getBooks');
  });
...
cy.request({
  method: 'GET',
  url: 'http://localhost:5173/all-books',
  headers: {
